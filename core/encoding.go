...
type GobAccoutEncoder struct {
	w io.Writer
}

func NewGobAccountEncoder(w io.Writer) *GobAccoutEncoder {
	return &GobAccoutEncoder{w: w}
}

func (enc *GobAccoutEncoder) Encode(a *Account) error {
	return gob.NewEncoder(enc.w).Encode(a)
}

type GobAccoutDecoder struct {
	r io.Reader
}

func NewGobAccountDecoder(r io.Reader) *GobAccoutDecoder {
	return &GobAccoutDecoder{r: r}
}

func (enc *GobAccoutDecoder) Decode(a *Account) error {
	return gob.NewDecoder(enc.r).Decode(a)
}
